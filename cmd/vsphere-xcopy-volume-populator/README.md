...
**Important**: For HPE Primera/3PAR, the `STORAGE_HOSTNAME` must include the full URL with protocol and the 3PAR's Web Services API (WSAPI) port (e.g., `https://192.168.1.1:8080`). Depending on configured certificates you may need to skip SSL verification.
