...
func TestParseVmdkPath(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedVMDisk: populator.VMDisk{VMName: "vm-1", Datasource: "mydatastore", VmdkFile: "disk-1.vmdk"},
			expectError:    false,
		},
		{
			name:           "Valid VMDK Path with spaces",
			vmdkPath:       "[my datastore] my vm/my vm-disk-1.vmdk",
			expectedVMDisk: populator.VMDisk{VMName: "my vm", Datasource: "my datastore", VmdkFile: "my vm-disk-1.vmdk"},
			expectError:    false,
		},
		{
