...
func (r *Client) createDiskSnapshotsWithPrefix(vmRef ref.Ref, prefix string) ([]string, error) {
	...
	for i, diskID := range diskIDs {
		...
		if err != nil {
			r.cleanupSnapshots(ctx, snapshotClient, snapshotRG, snapshotNames)
			return nil, liberr.Wrap(err)
		}
...
func (r *Client) IsSnapshotReady(snapshotName string) (bool, error) {
	...
	ready, err := snapshotProvisioningReady(*result.Properties.ProvisioningState)
	if err != nil {
		return false, liberr.Wrap(err)
	}
	return ready, nil
...
func (r *Client) cleanupSnapshots(ctx context.Context, client SnapshotAPI, resourceGroup string, snapshotNames []string) {
	for _, name := range snapshotNames {
		if err := r.deleteSnapshot(ctx, client, resourceGroup, name); err != nil {
			log.Error(err, "Failed to cleanup snapshot", "snapshot", name)
		}
	}
}

func snapshotProvisioningReady(state string) (bool, error) {
	switch state {
	case "Succeeded":
		return true, nil
	case "Failed", "Canceled":
		return false, fmt.Errorf("snapshot provisioning %s", strings.ToLower(state))
	default:
		return false, nil
	}
}
