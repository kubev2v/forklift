...
func (m *MockPopulator) Populate(vmId, sourceVMDKFile string, persistentVolume populator.PersistentVolume, progress chan<- uint64, quit chan error, xcopyUsed chan<- int) error {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Populate", vmId, migrationHostId, sourceVMDKFile, persistentVolume, progress, quit, xcopyUsed)
	ret0, _ := ret[0].(error)
	return ret0
}

// Populate indicates an expected call of Populate.
func (mr *MockPopulatorMockRecorder) Populate(vmId, migrationHostId, sourceVMDKFile, persistentVolume, progress, quit, xcopyUsed any) *gomock.Call {
