	...
	for _, h := range hosts {
		...
		for _, wwn := range h.Wwn {
			for _, hostAdapter := range esxAdapters {
				if !strings.HasPrefix("fc.", hostAdapter) {
					continue
				}
				adapterWWN, err := fcUidToWWN(hostAdapter)
				if err != nil {
					klog.Warningf("failed to format ESX adapter %s to wwn %s", hostAdapter, err)
					continue
				}
...
// 'fc.2020202020202020:2020202020202020'
//
//	fc.    | 2020202020202020 | : | 2020202020202020
...
func fcUidToWWN(fcUid string) (string, error) {
	...
	if len(formattedWwn) == 0 {
		return "", fmt.Errorf("fcUid %q should be in the form of 'fc.2020202020202020:2020202020202020' or 'fc.2020202020202020'", fcUid)
	}
	formattedWwn = strings.Split(formattedWwn, ":")[0]
