...
func (r *Collector) collectNetworks(ctx context.Context) error {
	...
	for _, vnet := range vnets {
		...
		resourceGroup, err := auth.ParseResourceGroup(*vnet.ID)
		if err != nil {
			r.log.Error(err, "Failed to parse VNet resource group", "vnet", *vnet.Name)
			continue
		}

		subnets, err := r.client.ListSubnets(ctx, resourceGroup, *vnet.Name)
		if err != nil {
