...
* Support the existing basic VM-start check only when the user selects a
  dedicated remote validation namespace.
...
* Creating VMs, PVCs, snapshots, migrations, or other workload resources on
  the remote target except for the explicitly selected workload check in its
  dedicated validation namespace.
...
* Do not expose image, shell commands, raw VCV include patterns, or target
  service account in the user API. Expose a target namespace only for the
  reviewed workload check.
...
approved catalog, and publish report snapshots. The upstream must-gather
entrypoint should not be used unchanged for this Job.

`platform` is the default public check ID. It expands to these reviewed VCV paths rather than
...
The next workload increment adds a separate `workload` check ID. It runs the
existing VCV basic VM-start check only when `spec.validationNamespace` names a
dedicated remote namespace. The controller passes a five-minute VM-ready
timeout; the Job's 15-minute active deadline remains the outer bound. The
namespace must contain no user workloads because VCV removes the generated VM,
VMI, DataVolumes, and PVCs on normal completion, failure, or termination.
...
* The Job authenticates to the remote target without a pre-existing `oc login`.
  It creates remote workload resources only for the explicit workload check
  and removes them before completion.
