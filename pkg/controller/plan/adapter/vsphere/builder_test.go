...
var _ = Describe("vSphere builder", func() {
	Context("PopulatorVolumes", func() {
		...
		It("should set migrationHost when dedicatedMigrationHosts is set", func() {
			// Setup
			builder := createBuilder(
				&core.Secret{
					ObjectMeta: meta.ObjectMeta{Name: "test-secret", Namespace: "test"},
			...
			builder.Source.Provider.Spec.Settings = map[string]string{
				// also put some spaces in the string
				v1beta1.DedicatedOffloadMigrationHosts: "host1 ,host2 ",
			}
			vm := model.VM{
				VM1: model.VM1{
					VM0: model.VM0{
