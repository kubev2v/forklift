...
if (-not $groupedRoutes) {
    ...
    foreach ($group in $gatewayDuplicates) {
        ...
        # If all unique interfaces in this group are active, this is a legitimate
        # multi-homed configuration (same gateway reachable via multiple NICs) -- leave it alone.
        $uniqueIFs = $routes | Select-Object -ExpandProperty InterfaceIndex -Unique
        $activeIFCount = 0
        foreach ($ifIdx in $uniqueIFs) {
            if (Get-NetAdapter | Where-Object { $_.InterfaceIndex -eq $ifIdx } -ErrorAction SilentlyContinue) {
                $activeIFCount++
            }
        }
        if ($uniqueIFs.Count -gt 1 -and $activeIFCount -eq $uniqueIFs.Count) {
            Write-Host "  Skipping group '$($group.Name)': all $activeIFCount unique interfaces are active (multi-homed)" -ForegroundColor Gray
            continue
        }
...
foreach ($gateway in $currentDefaultGateways) {
    ...
    try {
        Set-ItemProperty -Path $regPath -Name "DefaultGateway" -Value $merged -Type MultiString -ErrorAction Stop
        Write-Host "    [OK] Gateway written to registry for $interfaceAlias" -ForegroundColor Green
    } catch {
        Write-Host "    [FAILED] Could not write DefaultGateway to registry for $interfaceAlias`: $($_.Exception.Message)" -ForegroundColor Red
    }
