...
type Migration struct {
	...
	PopulatorContainerLimitsCpu      string
	PopulatorContainerLimitsMemory   string
	PopulatorContainerRequestsCpu    string
	PopulatorContainerRequestsMemory string
...
func (r *Migration) Load() (err error) {
	...
	if val, found := os.LookupEnv(PopulatorContainerLimitsCpu); found {
		r.PopulatorContainerLimitsCpu = val
	} else {
		r.PopulatorContainerLimitsCpu = "1000m"
	}
	if val, found := os.LookupEnv(PopulatorContainerLimitsMemory); found {
		r.PopulatorContainerLimitsMemory = val
	} else {
		r.PopulatorContainerLimitsMemory = "1Gi"
	}
	if val, found := os.LookupEnv(PopulatorContainerRequestsCpu); found {
		r.PopulatorContainerRequestsCpu = val
	} else {
		r.PopulatorContainerRequestsCpu = "100m"
	}
	if val, found := os.LookupEnv(PopulatorContainerRequestsMemory); found {
		r.PopulatorContainerRequestsMemory = val
	} else {
		r.PopulatorContainerRequestsMemory = "512Mi"
	}
