...
func (r *Builder) resolveSnapshotClassName() (string, error) {
	...
	err := r.Destination.List(context.TODO(), vscList, &client.ListOptions{})
	if err != nil {
...
func (r *Builder) BuildVolumeSnapshotContent(vmRef ref.Ref, snapshotResourceID string, diskIndex int) (*snapshotv1.VolumeSnapshotContent, error) {
	...
	vsc := &snapshotv1.VolumeSnapshotContent{
		ObjectMeta: meta.ObjectMeta{
			Name:        vscName,
			Labels:      labels,
			Annotations: map[string]string{azure.AnnSourceID: r.vmARMID(vmRef)},
		},
	...
	pvc := &core.PersistentVolumeClaim{
		ObjectMeta: meta.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				azure.AnnSourceID:   r.vmARMID(vmRef),
...
func (r *Builder) ResolvePersistentVolumeClaimIdentifier(pvc *core.PersistentVolumeClaim) string {
	if pvc.Labels != nil {
		return pvc.Labels[azure.LabelDiskIndex]
	}
	return ""
}
...
func (r *Builder) vmARMID(vmRef ref.Ref) string {
	if auth.IsARMResourceID(vmRef.ID) {
		return vmRef.ID
	}

	azureVM, err := inventory.GetAzureVM(r.Source.Inventory, vmRef)
	if err == nil && azureVM.ID != "" {
		return azureVM.ID
	}
	...
	vmName := vmRef.Name
	if vmName == "" {
		vmName = auth.ParseResourceName(vmRef.ID)
	}
