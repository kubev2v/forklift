...
request and its results; a short-lived Job on the Forklift hub executes the
checks against the target API.
...
* When a Plan has selected a namespace, storage mappings, and
...
* Copying a Provider bearer token or kubeconfig to a target-cluster Pod.
...
* **Plan profile** includes Provider checks and runs disposable workload probes
  ...
  namespace, destination StorageClass, snapshot support, and
...
spec:
  ...
  targetNamespace: cluster-abc-1-target # remote namespace; must equal Plan target
  checks:
    - platform
    - vm-start
    - live-migration
...
`targetNamespace` names the remote namespace in which a mutating workload
probe operates; it is not the namespace in which the validator Job runs.
`cleanupPolicy` controls only those temporary target workload resources. The
hub Job, credential Secret, and report resources remain controller-owned and
are never retained by a target-workload debugging policy.

API validation rules must enforce the following:

* `providerRef` is required and must resolve to an OpenShift Provider.
* The validation, referenced Provider, and referenced Provider credential
  Secret are namespace-local to the Forklift installation namespace. A
  validation cannot use a cross-namespace Provider or credential Secret.
...
* A Provider profile may request only its explicitly supported catalog. A
  mutating Provider check requires a dedicated remote validation namespace.
...
before `v1`. The result list must be bounded: detailed logs belong to the
hub-local Job, while CR status carries a concise summary and references.
...
the policy required for it. This avoids unexpected Jobs on every Provider
created by an existing installation.

The validation CR owns its hub-local Job, credentials Secret, publishing
ServiceAccount, narrow Role/RoleBinding, and report ConfigMap through normal
same-cluster owner references. Target workload resources cannot have a hub
owner reference; VCV labels them with the validation UID and cleans them up.
...
| `ValidationFailed` | Critical for a required Plan check; Warn for an advisory Provider check | At least one required check failed, or the Job/result contract could not complete. `Items` contains failed check IDs. |
...
| `ValidationInputInvalid` | Critical | References, profile constraints, or requested checks cannot be resolved. No hub Job is created. |
...
The provider-level action selects an approved profile/check catalog and, for a
mutating workload check, a dedicated remote validation namespace. The UI must
show that a Provider result does not prove a specific Plan's storage or
namespace. It must not promise that the action can run solely because the
caller can view a Provider; the UI should preflight `create` permission for the
CR and show the server-side admission error when the target policy rejects a
request.
...
after target namespace and storage-map selection, say that a disposable
VM-based validation will run and list its expected resources and required
permissions. It should not run a check on every field change. Create or refresh
the Plan validation only after the user saves a valid Plan (or uses an explicit
...
link only to hub-local Job and report ConfigMap artifacts; remote credentials
and target workload resources remain controller concerns.
...
sequenceDiagram
    ...
    participant Job as Validator Job<br/>(hub namespace)
    participant Result as hub result ConfigMap<br/>(CTRF snapshots)
    ...
    VC->>Job: Create owned Job, credential Secret, and result ConfigMap
    Job->>Target: Authenticate with Provider URL, token, and TLS settings
    Job->>Job: Run selected PoC and MTV checks
    Job->>Target: Create VM, migration, snapshot/restore as selected
    Job->>Result: Publish complete versioned CTRF snapshots
    VC->>Result: Watch and validate progressive snapshots
    VC->>CR: Persist per-check results and phase
    alt Provider profile
        VC->>Projection: Project availability/advisory conditions
    else Plan profile
        VC->>Projection: Project pending, warning, or blocking conditions
    end
    Job->>Target: Clean up labeled test resources
    VC->>Job: Clean up owned hub resources
```

The hub Job has two deliberately separate identities. It mounts a per-run,
read-only Provider credential and turns the Provider URL, bearer token, and
TLS settings into a short-lived remote kubeconfig for VCV. Its hub
ServiceAccount has only `get`, `patch`, and `update` on the controller-created
result ConfigMap for that run. The Provider credential is never copied to the
target cluster or exposed in status, logs, or Pod environment variables.
...
ServiceAccount, result artifact, and validator image remain on the hub. This
does not require installing or synchronizing Forklift resources on the spoke.
The existing Provider identity authorizes VCV's remote API calls; target
workload resources are created only for selected checks. Hub-to-spoke API
connectivity is already required for MTV. The hub Job must also receive the
Provider CA, or the explicit insecure-TLS setting, to trust the target API.

The runner publishes complete, bounded, versioned CTRF snapshots to the
hub-local result ConfigMap. Parsing Pod logs is not the primary API: logs are
lossy, have retention policies outside Forklift's control, and are awkward for
controllers. The ConfigMap is a per-run report mailbox, not a lifecycle API:
the controller validates snapshots, owns CR and Provider/Plan status, and may
retain only a small diagnostic excerpt in status.
...
* authorization preflight for the effective Provider identity.
...
1. Verify that the effective Provider identity has the minimum actions required
...
mutatesTarget: true
inputs: [targetNamespace, storageClass]
...
* Hub Job templates, stable labels, cleanup/retry behavior, result parsing,
...
There are two principals with deliberately different scopes:

* **Hub validator Job's Provider credential:** authorizes VCV's direct remote
  API calls. It is mounted read-only into the short-lived hub Job and needs the
  selected check's remote permissions (VM, VMI migration, snapshot/restore,
  PVC/DataVolume, and relevant discovery APIs).
* **Hub validator Job ServiceAccount:** has only `get`, `patch`, and `update`
  on its one controller-created result ConfigMap. It cannot create arbitrary
  ConfigMaps or write Forklift CR status.

Forklift should not install a validator identity or broad RoleBinding in a
customer target namespace. The Provider credential remains in the Forklift
trust domain; it is never copied to the target cluster.

### Resource creation and cleanup

Tests create real workloads and could consume quota or leave resources behind.
Every target test resource is labeled with validation UID, scope, and expiry
metadata. The composed check installs traps and removes resources in reverse
order. The controller independently removes its owned hub Job, credential
Secret, ServiceAccount, Role, RoleBinding, and result ConfigMap on input change
or CR deletion. A later remote-cleanup safety net may reconcile labels after a
runner failure; it is not a reason to move the Job to the target.
...
The hub cluster must be able to pull the validator image. Image pull
...
bandwidth. The controller must acquire a hub-local Lease keyed by the Forklift
namespace and Provider before creating a Job. The validation UID is the holder
identity; the holder renews while its Job is active, releases on terminal state
or deletion, and expiry permits recovery after a controller failure. A
contending request reports `ValidationAlreadyRunning` and must not replace the
holder's Provider/Plan projection. Future Plan policy can additionally
serialize validations per target namespace or impose a global limit. Timeouts
apply to the overall run and each operation; the PoC's per-check timeout alone
is insufficient for controller lifecycle control.
...
2. Prove that the PoC container can run as a hub Job using the Provider URL,
   token, and TLS settings, and produces progressive hub-local CTRF snapshots.
3. Verify VM, VMI migration, snapshot, and restore manifests against every
   supported OpenShift Virtualization version.
4. Define the minimum remote permissions for the Provider identity and the
   narrow hub Role used to publish the result ConfigMap.
...
2. Add Provider-profile authorization preflight and the explicitly selected,
   dedicated-namespace workload probe.
...
5. Add the per-Provider Lease and optional automatic Provider trigger behind a
...
4. Add namespace/global concurrency policy and metrics on top of the
   per-Provider Lease.
...
condition mapping, profile policy, hub Job generation, CTRF parsing, cleanup
...
* Plan input change (StorageMap, target namespace, validation
...
Downgrading removes the controller but can leave hub Jobs and target workload
resources if an active run exists. Upgrade documentation must instruct operators
to wait for active runs to finish or delete their `VirtualizationValidation` CRs
first. Target workload resources remain discoverable by their labels and can be
removed with a provided cleanup procedure. The CRD should be retained during
rollback unless the user explicitly removes it after confirming no active runs.

## Drawbacks

This adds a CRD, controller, image, release dependency, hub Job lifecycle, and
remote API permissions model to Forklift. It also performs real operations on a
...
### 6. Run a target-cluster validation Job or copy the Provider credentials there

This would spread Forklift-managed execution resources and Provider credentials
to every target while providing no benefit for VCV's API-driven checks. The
proposed split keeps the Job and credential on the hub and uses the existing
Provider identity only for remote API calls.
...
* A ConfigMap update has no unambiguous association with a particular hub
...
* ConfigMap size limits make one shared object unsuitable for full CTRF history
  or diagnostics. A per-run, size-bounded report mailbox is acceptable; storing
  logs or unbounded history risks exhaustion and accidental exposure of target
  details.
...
4. Which documented remote permissions must the existing Provider identity
   have for each supported check, and should Forklift ship RBAC examples?
5. What result artifact retention period balances debuggability and namespace
   cleanliness?
6. What is the supported behavior when the hub cluster cannot pull the
   Forklift validation image?
