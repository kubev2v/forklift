...
func (h *HostLeaseLocker) WithLock(ctx context.Context, hostID string, work func() error) error {
	if hostID == "" {
...
func (h *HostLeaseLocker) executeWorkWithLease(
		...
		for {
			...
			case <-ticker.C:
				...
				if err != nil {
					...
					select {
					case renewalErrors <- fmt.Errorf("failed to renew lease: %w", err):
					default:
					}
					return
				}
	...
	klog.Infof("Work complete. Releasing lock slot %d for host %s", slot, hostID)

	// Delete the lease to release the lock (even if work failed)
	deleteCtx, deleteCancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer deleteCancel()
	deleteErr := leaseClient.Delete(deleteCtx, lease.Name, metav1.DeleteOptions{})
	if deleteErr != nil {
		klog.Warningf("Failed to delete lease %s: %v", lease.Name, deleteErr)
	} else {
		klog.Infof("Successfully released lock slot %d for host %s", slot, hostID)
	}
