...
func (p *PowermaxClonner) EnsureClonnerIgroup(initiatorGroup string, clonnerIqn []string) (populator.MappingContext, error) {
	...
	// create a masking view, add the storage group to it - name it with the same name
	// create InitiatorGroup on the masking view
	// add clonnerIqn to that initiar group
	// add port group with protocol type that match the cloner IQN type
	...
	if e, ok := err.(*pmxtypes.Error); ok && e.HTTPStatusCode == 404 {
		...
		if err != nil {
			klog.Errorf("failed to create gropup %v ", err)
			return nil, err
		}
	}

	//change info message
	klog.Infof("storage group %+v", sg)
	...
	for _, host := range hg.Hosts {
		...
		klog.Infof("host %s initiators %v", host.HostID, host.Initiators)
		for _, iqn := range clonnerIqn {
	...
	pgs, err := p.client.GetPortGroupList(ctx, p.symmetrixID, protocolType)
	if err != nil {
		return nil, err
	}
	klog.Infof("port groups %+v", pgs.PortGroupIDs)

	portGroupID := ""
	for _, pgId := range pgs.PortGroupIDs {
		pg, err := p.client.GetPortGroupByID(ctx, p.symmetrixID, pgId)
		if err != nil {
			return nil, err
		}
		klog.Infof("port group protocol %+v", pg.PortGroupProtocol)
		portGroupID = pg.PortGroupID
	}

	//change info message
	klog.Infof("host group %+v", hg)
...
func (p *PowermaxClonner) Map(initiatorGroup string, targetLUN populator.LUN, mappingContext populator.MappingContext) (populator.LUN, error) {
	klog.Infof("mapping volume %s to %v", targetLUN.ProviderID, initiatorGroup)
	ctx := context.TODO()
	...
	if err != nil {
		...
		klog.Infof("failed mapping volume %s to %v: %v", targetLUN.ProviderID, initiatorGroup, err)
		return targetLUN, err
	}
	...
	klog.Infof("successfully mapped volume %s to %v with masking view %s", targetLUN.ProviderID, initiatorGroup, mv.MaskingViewID)
	return targetLUN, err
}

// ResolveVolumeHandleToLUN implements populator.StorageApi.
func (p *PowermaxClonner) ResolveVolumeHandleToLUN(volumeHandle string) (populator.LUN, error) {
	ctx := context.TODO()
	volID := volumeHandle[strings.LastIndex(volumeHandle, "-")+1:]
	volume, err := p.client.GetVolumeByID(ctx, p.symmetrixID, volID)
	if err != nil {
		return populator.LUN{}, fmt.Errorf("failed getting details for volume %s: %v", volID, err)
	}
	return populator.LUN{Name: volume.VolumeIdentifier, ProviderID: volume.VolumeID, NAA: volume.WWN}, nil
