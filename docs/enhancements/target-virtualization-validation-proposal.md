...
actual target namespace and storage path. A short-lived Job
on the Forklift hub executes a trusted validator image based on
...
* Preserve least privilege: never copy a Provider credential to a target
  cluster.
...
spec:
  ...
  targetNamespace: cluster-abc-1-target # remote workload namespace
...
Status records a phase, observed input hash, run ID, hub Job and result
ConfigMap references, bounded per-check results, and result-artifact
references. `runNonce` triggers an explicit rerun. Automatic runs use an input
hash so a change to relevant Provider/Plan/storage/image inputs invalidates an
old result.
...
The validation, referenced Provider, and referenced Provider credential Secret
are namespace-local to the Forklift installation namespace; cross-namespace
Provider and credential references are rejected.
...
| Plan | User or Plan controller | Provider checks plus one disposable VM using the Plan's namespace and selected storage | Required checks block Plan Ready and execution |

Automatic Provider validation is opt-in. Existing Provider behavior does not
change by default.
...
sequenceDiagram
    ...
    participant Result as hub result ConfigMap
    ...
    Controller->>Job: Create owned Job, credential Secret, and result ConfigMap
    Job->>Target: Authenticate using Provider URL, token, and TLS settings
    Job->>Target: Run selected checks and workload probes
    Job->>Result: Publish complete CTRF snapshots
    Controller->>Result: Watch and validate progressive snapshots
    Controller->>CR: Persist phase and per-check status
    Controller->>Status: Project Provider or Plan summary
    Job->>Target: Clean up labeled workload resources
    Controller->>Job: Clean up owned hub resources
```

Forklift creates the Job, per-run credential Secret, ServiceAccount, narrow
Role/RoleBinding, and result ConfigMap in its own namespace. The hub Job mounts
the Provider credential read-only and constructs a short-lived remote
kubeconfig from the Provider URL, token, and TLS settings. It uses its separate
hub ServiceAccount only to patch the one controller-created result ConfigMap.
The Provider credential is never copied to the target cluster. Cross-cluster
owner references are not possible, so target workload resources are labeled
with the validation UID and cleaned up by the runner.
...
validator Job, its report ConfigMap, and its publishing RBAC remain on the hub.
No Forklift resource, validator image, or Provider credential is installed on
the spoke. The existing Provider identity authorizes remote API calls made by
VCV. For workload checks, only disposable test resources are created in the
selected target namespace. Hub-to-spoke API connectivity is already a
prerequisite for MTV; custom Provider CAs must be propagated to the hub Job.
...
Before creating a validator Job, the controller acquires a hub-local Lease
keyed by the Forklift namespace and Provider. The validation UID is the holder
identity. The holder renews while its Job is active and releases the Lease on
terminal completion or CR deletion; Lease expiry permits recovery after a
controller failure. A competing request reports `ValidationAlreadyRunning` and
must not overwrite the active run's Provider or Plan status projection. Future
Plan policy can add target-namespace or installation-wide limits on top of this
...
* Single-node clusters cannot demonstrate live migration. That result must be
  ...
  VM-start failure.
* Quota, SCC/PSA, admission, hub image pull, and remote RBAC failures must
  retain the API error and failing check ID in status.
* Test resources create real cost and load. A hub-local Lease serializes runs
  per Provider; future Plan policy can additionally serialize per target
  namespace. Time-box each operation and the whole run, and label all target
  resources for cleanup and debugging.
* Detailed results belong in a versioned hub-local report artifact; status
  remains small.
...
The feature is disabled by default. Phase one delivers the stand-alone CR,
Provider authorization checks, and an explicitly selected dedicated-namespace
workload probe; phase two adds Plan-triggered workload validation and the
readiness gate. Unit tests cover status/input-hash/retry/cleanup logic;
