...
func (test *TestFramework) setupOpenShiftEnvironment(t *testing.T, result *TestResult) {
	...
	if test.Config.TargetVMName != "" {
		...
		step2.Complete("Passed", fmt.Sprintf("Using existing storage secret: %s", storageSecretName))
	} else {
		step2 := result.Step("Create storage secret")
		randSuffix, _ := helpers.GenerateRandomString(4)
		storageSecretName := fmt.Sprintf("%s-storage-secret-%s", test.vmName, randSuffix)
		_ = os.Setenv("STORAGE_SECRET_NAME", storageSecretName)
		...
		step2.Complete("Passed", fmt.Sprintf("Storage secret created: %s", storageSecretName))
	}
