	...
	for depth > 0 {
		...
		case xml.StartElement:
			...
			case "backup":
				...
				builder.Backup(v)
			case "backup_mode":
				vp, err := XMLDiskBackupModeReadOne(reader, &t)
				v := *vp
				if err != nil {
					return nil, err
				}
				builder.BackupMode(v)
			case "bootable":
				v, err := reader.ReadBool(&t)
				if err != nil {
					return nil, err
...
func XMLDiskBackupModeReadOne(reader *XMLReader, start *xml.StartElement) (*DiskBackupMode, error) {
	if start == nil {
		st, err := reader.FindStartElement()
		if err != nil {
			if err == io.EOF {
	...
	result := new(DiskBackupMode)
	*result = DiskBackupMode(s)
	return result, nil
}

func XMLDiskBackupModeReadMany(reader *XMLReader, start *xml.StartElement) ([]DiskBackupMode, error) {
	if start == nil {
		st, err := reader.FindStartElement()
		if err != nil {
			if err == io.EOF {
	...
	for depth > 0 {
		...
		case xml.StartElement:
			...
			results = append(results, DiskBackupMode(one))
		case xml.EndElement:
			depth--
		}
	}
