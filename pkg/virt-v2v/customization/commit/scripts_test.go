...
func TestScripts_EmptyDisks(t *testing.T) {
	t.Parallel()
	fb := newFakeBuilder(nil)
	actions := []api.ExecAction{{Type: api.ActionFirstboot, Value: "/tmp/script.sh"}}
...
func TestScripts_UnsupportedActionType(t *testing.T) {
	t.Parallel()
	fb := newFakeBuilder(nil)
	actions := []api.ExecAction{{Type: "unknown", Value: "foo"}}
...
func TestScripts_ValidActions(t *testing.T) {
	...
	actions := []api.ExecAction{
		{Type: api.ActionFirstboot, Value: "/tmp/first.sh"},
		{Type: api.ActionRun, Value: "/tmp/run.sh"},
	}
...
func TestScripts_RunError(t *testing.T) {
	...
	actions := []api.ExecAction{{Type: api.ActionFirstboot, Value: "/tmp/s.sh"}}
	err := Scripts(fb, []string{"/disk1"}, nil, actions)
	if err == nil {
