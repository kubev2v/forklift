...
var ErrNoNetworkMapping = errors.New("no valid network mapping found")
...
func (r *Builder) mapNetworks(vm *model.VM, object *cnv.VirtualMachineSpec) (err error) {
	...
	for _, nic := range vm.NICs {
		mapped, err := r.findNetworkMapping(nic, netMapIn)
		if err != nil {
			if errors.Is(err, ErrNoNetworkMapping) {
				r.Log.Info("No valid network mapping found for NIC", "NIC", nic)
				continue
			}
...
func (r *Builder) findNetworkMapping(nic vsphere.NIC, netMap []api.NetworkPair) (*api.NetworkPair, error) {
	for i := range netMap {
		candidate := &netMap[i]
	...
	return nil, ErrNoNetworkMapping
}
