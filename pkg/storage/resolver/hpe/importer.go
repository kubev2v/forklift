...
func (i *HpeImporter) resolveVVol(vVolId string) (map[string]string, bool, error) {
	...
	if !found {
		klog.V(2).InfoS("HPE queried WSAPI, VVol not found on this array (cross-array), deferring to xcopy", "vVolId", vVolId)
...
func (i *HpeImporter) resolveRDM(deviceName string) (map[string]string, bool, error) {
	...
	if !found {
		klog.V(2).InfoS("HPE queried WSAPI, RDM not found on this array (cross-array), deferring to xcopy", "deviceName", deviceName)
		return nil, false, nil
	}
	return map[string]string{AnnotationKey: name}, true, nil
}
