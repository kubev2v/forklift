...
func (h *DiskHandler) AddRoutes(e *gin.Engine) {
	root := h.Config.ProviderRoot() + "/" + DiskCollection
	e.GET(root, h.List)
	e.GET(root+"/", h.List)
	e.GET(root+"/:"+DiskParam, h.Get)
}
...
// Build self link (URI).
func (r *Disk) Link(p *api.Provider, cfg Config) {
	r.SelfLink = base.Link(
		cfg.ProviderRoot()+"/"+DiskCollection+"/:"+DiskParam,
		base.Params{
