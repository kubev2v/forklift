...
var _ = Describe("buildCsiImportPVC", func() {
	...
	It("should defer to xcopy when the storage API is unreachable (not fail the migration)", func() {
		builder := createBuilder()
		disk := vvolDisk
	...
	It("should return nil (defer) when volume not found, regardless of this entry's own xcopy config", func() {
		// validateOffloadPlugin guarantees a fallback exists elsewhere in the StorageMap;
		// buildCsiImportPVC doesn't need to re-check it on the resolved entry itself.
