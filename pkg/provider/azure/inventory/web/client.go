...
func (r *Finder) findTypedResource(resource interface{}, ref base.Ref) (err error) {
	if ref.ID != "" && auth.IsARMResourceID(ref.ID) {
		if err = r.Get(resource, ref.ID); err == nil {
			return nil
		}
	...
	// which are incompatible with HTTP path parameters. Fall back to name-based list lookup.
	name := ref.Name
	if name == "" && ref.ID != "" {
