...
func (h *VMHandler) AddRoutes(e *gin.Engine) {
	root := h.Config.ProviderRoot() + "/" + VMCollection
	e.GET(root, h.List)
	e.GET(root+"/", h.List)
...
// Build self link (URI).
func (r *VM) Link(p *api.Provider, cfg Config) {
	r.SelfLink = base.Link(
		cfg.ProviderRoot()+"/"+VMCollection+"/:"+VMParam,
		base.Params{
