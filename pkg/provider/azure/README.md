...
| `resourceGroup` | **No** | Azure resource group containing the source VMs. When omitted, inventory covers all resource groups in the subscription and `snapshotResourceGroup` must be set on the provider. |

### Required Azure RBAC Roles

The service principal needs these roles scoped to the **source resource group** or, when `resourceGroup` is omitted from the secret, at **subscription** scope:
...
When `resourceGroup` is omitted from the secret, set `spec.settings.snapshotResourceGroup` to the resource group where Forklift should create snapshots. Plans should reference VMs by full ARM ID when VM names are not unique across resource groups.
...
kubectl mtv create provider my-azure --type azure \
  ...
  --azure-client-id "$AZURE_CLIENT_ID" \
  --azure-client-secret "$AZURE_CLIENT_SECRET" \
  --azure-snapshot-resource-group "snapshot-rg"
```
...
| `--azure-resource-group` | **No** | Resource group containing the source VMs. Omit to discover all resource groups in the subscription. |
...
stringData:
  ...
  # resourceGroup: "my-resource-group"  # optional; omit for subscription-wide inventory
```
...
|---------|----------|-------------|
| `snapshotResourceGroup` | No* | Resource group for snapshots (defaults to the secret's `resourceGroup`). Required when `resourceGroup` is omitted from the secret. |
