...
func isDefaultRoute(network string) bool {
	return network == "0.0.0.0" || network == "::" || network == "::/0"
}
...
//
// Returns:
//    - The selected gateway IP as a string.
//    - An empty string if no suitable gateway is found.
...
func (r *Builder) mapMacStaticIps(vm *model.VM) (ipMap string, err error) {
	...
	// Sort with IPv4 first for virt-v2v backward compatibility
	sortedNetworks := sortNetworksByIPFamily(vm.GuestNetworks)

	for _, network := range sortedNetworks {
		if !shouldIncludeNetwork(network, isWindowsFlag) {
			continue
		}

		gateway := selectGateway(network.IP, network.Device, vm.GuestIpStacks, isWindowsFlag)

		// Windows requires a gateway for static IP configuration.
		// Linux can configure IPs without gateways (uses routing table).
		if isWindowsFlag && gateway == "" {
			continue
		}

		config := formatNetworkConfig(network, gateway)
...
func detectProviderIPFamily(provider *api.Provider) bool {
	if providerURL, err := url.Parse(provider.Spec.URL); err == nil {
