...
udev_from_ifcfg() {
    ...
    do
        ...
        # Find the matching network script file (check both IPv4 and IPv6 patterns)
        IFCFG=$(grep -l -F "$S_IP" "$NETWORK_SCRIPTS_DIR"/*)
        if [ -z "$IFCFG" ]; then
...
udev_from_nm_dhcp_lease() {
    ...
    do
        ...
        LEASE_FILES=$(grep -l -F "$S_IP" "$NM_LEASES_DIR"/*.lease)
    ...
    find_interface_by_ip() {
        ...
        if netplan_supports_get; then
          ...
          netplan_get ethernets | grep -Eo "^[^[:space:]]+[^:]" | while read -r IFNAME; do
            ...
            NM_FILE=$(grep -l -F "$S_IP" "$SYSTEMD_NETWORK_DIR"/*)
            if [ -z "$NM_FILE" ]; then
