...
func TestGetSourceVolume_PanicsWhenClientIsNil(t *testing.T) {
	cloner := &VantaraCloner{}

	var c vmware.Client = nil
	...
	_, _ = cloner.getSourceVolume(
		c,
		"dummy-vm",
		populator.VMDisk{VmdkFile: "[ds] vm/vm.vmdk"},
	)
}

func TestMatchesVMDKPath_ExactMatch(t *testing.T) {
	v := &VantaraCloner{}

	fileName := "[datastore1] vm-folder/mydisk.vmdk"
	vmDisk := populator.VMDisk{
		VmdkFile: "/some/other/path/mydisk.vmdk",
	}

	if !v.matchesVMDKPath(fileName, vmDisk) {
		t.Fatalf("expected true, got false")
	}
}

func TestMatchesVMDKPath_DifferentFileName(t *testing.T) {
	v := &VantaraCloner{}

	fileName := "[datastore1] vm-folder/disk1.vmdk"
	vmDisk := populator.VMDisk{
		VmdkFile: "/some/path/disk2.vmdk",
	}
...
func TestMatchesVMDKPath_DifferentDirectorySameBase(t *testing.T) {
	v := &VantaraCloner{}

	fileName := "[datastore1] vm-folder/subdir/disk.vmdk"
	vmDisk := populator.VMDisk{
		VmdkFile: "disk.vmdk",
	}

	if !v.matchesVMDKPath(fileName, vmDisk) {
		t.Fatalf("expected true for same base filename")
	}
}

func TestMatchesVMDKPath_CaseSensitive(t *testing.T) {
	v := &VantaraCloner{}

	fileName := "[datastore1] vm-folder/DISK.VMDK"
	vmDisk := populator.VMDisk{
		VmdkFile: "disk.vmdk",
	}

	// filepath.Base 比較は大文字小文字を区別する
	if v.matchesVMDKPath(fileName, vmDisk) {
		t.Fatalf("expected false due to case sensitivity")
	}
}

func TestMatchesVMDKPath_EmptyInputs(t *testing.T) {
	v := &VantaraCloner{}
	...
	if !v.matchesVMDKPath(fileName, vmDisk) {
		t.Fatalf("expected true when both basenames are empty")
	}
}
