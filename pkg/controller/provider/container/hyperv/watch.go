...
// Endpoints.
const (
	BaseEndpoint       = "/v1/data/io/konveyor/forklift/hyperv/"
...
func (r *VMEventHandler) validated(batch []*policy.Task) {
	...
	if err != nil {
		r.log.Error(err, "Tx begin failed.")
		return
	}
	defer func() {
		_ = tx.End()
	...
	for _, task := range batch {
		...
		if err != nil {
			r.log.Error(err, "Tx update failed.")
			continue
		}
		if task.Error == nil {
			r.log.V(3).Info(
