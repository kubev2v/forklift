...
if ! python3 - "${VIRT_VALIDATE_URL}" <<'PY'
...
PY
then
    echo "ERROR: VIRT_VALIDATE_URL must be an absolute https URL" >&2
    exit 2
fi
...
publish_report() {
    ...
    if ! python3 - "${report_file}" "${patch_file}" "${max_report_bytes}" <<'PY'
import json
import sys
...
PY
    then
        echo "WARNING: unable to prepare validation report snapshot for publication" >&2
        return 1
    fi
    if ! oc --kubeconfig="${hub_kubeconfig}" -n "${configmap_namespace}" patch configmap \
        "${VIRT_VALIDATE_RESULT_CONFIGMAP}" --type=merge --patch "$(<"${patch_file}")" >/dev/null; then
        echo "WARNING: unable to publish validation report snapshot" >&2
        return 1
    fi
}
