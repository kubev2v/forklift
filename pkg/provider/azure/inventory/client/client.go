...
func New(provider *api.Provider, secret *core.Secret) (*Client, error) {
	...
	return &Client{
		...
		subscriptionScoped: creds.ResourceGroup == "",
	}, nil
}
...
func (c *Client) IsSubscriptionScoped() bool {
	return c.subscriptionScoped
}
...
func NewWithClient(azureClient AzureAPI, subscriptionID, resourceGroup string) *Client {
	return &Client{
		...
		subscriptionScoped: resourceGroup == "",
	}
}
...
func (c *Client) GetVMInstanceView(ctx context.Context, resourceGroup, vmName string) (*armcompute.VirtualMachineInstanceView, error) {
	return c.azureClient.GetVMInstanceView(ctx, resourceGroup, vmName)
}
...
func (c *Client) ListSubnets(ctx context.Context, resourceGroup, vnetName string) ([]*armnetwork.Subnet, error) {
	return c.azureClient.ListSubnets(ctx, resourceGroup, vnetName)
}
...
func (s *sdkClient) ListDisks(ctx context.Context, resourceGroup string) ([]*armcompute.Disk, error) {
	...
	if resourceGroup == "" {
		pager := s.diskClient.NewListPager(nil)
		for pager.More() {
			page, err := pager.NextPage(ctx)
			if err != nil {
...
func (s *sdkClient) ListVirtualNetworks(ctx context.Context, resourceGroup string) ([]*armnetwork.VirtualNetwork, error) {
	...
	if resourceGroup == "" {
		pager := s.vnetClient.NewListAllPager(nil)
		for pager.More() {
			page, err := pager.NextPage(ctx)
			if err != nil {
