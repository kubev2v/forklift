...
func (api *BlockStorageAPI) makeHTTPRequest(methodType, url string, body, headers map[string]string, authType, authValue string) (map[string]interface{}, error) {
	...
	if resp.StatusCode == http.StatusServiceUnavailable {
		...
		return api.makeHTTPRequest(methodType, url, body, headers, authType, authValue)
	}
...
func (api *BlockStorageAPI) checkUpdate(jobID string, headers map[string]string) (map[string]interface{}, error) {
	url := api.Job(jobID)
	return api.makeHTTPRequest("GET", url, nil, headers, "session", headers["Authorization"])
}
...
func (api *BlockStorageAPI) InvokeAsyncCommand(methodType, url string, body, headers map[string]string) (string, error) {
	result, err := api.makeHTTPRequest(methodType, url, body, headers, "session", headers["Authorization"])
	if err != nil {
		return "", err
	}
...
func (api *BlockStorageAPI) Connect() error {
	...
	userCreds := api.username + ":" + api.password

	// Check API version
	url := api.APIVersion()
	klog.Infof("Connecting to Vantara storage at %s", url)
	r, err := api.makeHTTPRequest("GET", url, nil, headers, "basic", userCreds)
	if err != nil {
	...
	r, err = api.makeHTTPRequest("POST", url, map[string]string{}, headers, "basic", userCreds)
	if err != nil {
...
func (api *BlockStorageAPI) Disconnect() error {
	...
	_, err := api.makeHTTPRequest("DELETE", url, nil, headers, "session", api.sessionToken)
	if err != nil {
...
func (api *BlockStorageAPI) GetLdev(ldevId string) (*LdevResponse, error) {
	...
	r, err := api.makeHTTPRequest("GET", url, nil, headers, "session", api.sessionToken)
	if err != nil {
