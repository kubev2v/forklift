...
var _ = Describe("vSphere builder", func() {
	...
	Context("getHostAddress", func() {
		DescribeTable("should return correct host address", func(host *model.Host, expected string) {
			...
			Entry("Both IPv4 and IPv6 (should prefer IPv6)",
				&model.Host{
					Resource: model.Resource{Name: "esxi-host.example.com"},
					Network: vsphere.HostNetwork{
						VNICs: []vsphere.VNIC{
			...
			Entry("ESXi host with IPv4 and global IPv6 (filters link-local, prefers IPv6)",
				&model.Host{
					Resource: model.Resource{Name: "esxi-host.example.com"},
					Network: vsphere.HostNetwork{
						VNICs: []vsphere.VNIC{
							{
								...
								IpV6Address: []string{
									...
									"2620:52:0:4948:3673:5aff:fe9a:dd78", // global IPv6 - should be used
								},
							},
						},
					},
				},
				"[2620:52:0:4948:3673:5aff:fe9a:dd78]", // Should prefer global IPv6 over IPv4
			),
		)
	})
