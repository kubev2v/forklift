...
func (c *Client) Finalize(vms []*planapi.VMStatus, planName string) {
	...
	// Verify REST client is available.
	if c.restClient == nil {
		c.Log.Error(nil, "REST client not initialized — cannot perform post-migration tagging")
		return
	}

	c.finalizeTagMigrated(context.TODO(), c.restClient, succeeded)
}
...
func (r *Client) connect() error {
	...
	// Initialize REST client for VAPI operations (tagging, etc.)
	r.restClient = rest.NewClient(vimClient)
	err = r.restClient.Login(context.TODO(), url.User)
	if err != nil {
		return liberr.Wrap(err)
	}
