...
const (
	...
	validationLeasePrefix = "virt-validation-provider-"
)

const (
	validationLeaseDuration      = 2 * time.Minute
	validationLeaseRenewInterval = 30 * time.Second
)
...
func Add(mgr manager.Manager) error {
	...
	if err := c.Watch(source.Kind(mgr.GetCache(), &api.VirtualizationValidation{},
		...
		predicate.TypedFuncs[*api.VirtualizationValidation]{
			UpdateFunc: func(event event.TypedUpdateEvent[*api.VirtualizationValidation]) bool {
				return validationChanged(event.ObjectOld, event.ObjectNew)
			},
		})); err != nil {
		return err
	}
...
func validationChanged(old, current *api.VirtualizationValidation) bool {
	return old.Generation != current.Generation ||
		controllerutil.ContainsFinalizer(old, ValidationFinalizer) != controllerutil.ContainsFinalizer(current, ValidationFinalizer)
}
...
func (r Reconciler) Reconcile(ctx context.Context, request reconcile.Request) (result reconcile.Result, err error) {
	...
	checks, workloadCheck, validationErr := providerProfileChecks(v.Spec.Checks, v.Spec.ValidationNamespace)
	...
	if providerNamespace != v.Namespace {
		r.failed(v, "ProviderNamespaceNotAllowed", "referenced Provider must be in the VirtualizationValidation namespace")
		return
	}
	...
	var leaseErr error
	defer func() {
		if !leaseHeld || validationInProgress(v) {
			return
		}
		if releaseErr := r.releaseProviderLease(ctx, v); releaseErr != nil {
			log.Error(releaseErr, "failed to release virtualization validation Lease",
				"validation", v.Name, "provider", v.Spec.ProviderRef.Name)
			if err == nil {
				err = releaseErr
			}
		}
	}()
	projectProviderStatus := true
	defer func() {
		if !projectProviderStatus {
			return
		}
		if projectionErr := r.projectProviderStatus(ctx, provider, v); projectionErr != nil {
			log.Error(projectionErr, "failed to project virtualization validation status to Provider",
				"validation", v.Name, "inputHash", v.Status.ObservedInputHash)
			if err == nil {
				err = projectionErr
			}
		}
	...
	if secretNamespace != v.Namespace {
		r.failed(v, "ProviderSecretNamespaceNotAllowed", "referenced Provider credential Secret must be in the VirtualizationValidation namespace")
		return
	}
	secret := &core.Secret{}
	...
	leaseHeld, leaseErr = r.acquireProviderLease(ctx, v)
	if leaseErr != nil {
		err = leaseErr
		return
	}
	if !leaseHeld {
		// The active holder alone projects the Provider condition. A rejected
		// request must not replace it with a terminal failure.
		projectProviderStatus = false
		r.failed(v, "ValidationAlreadyRunning", "a virtualization validation is already running for this Provider")
		return
	}
	name := resourceName(v, hash)
	if err = r.deleteSupersededResources(ctx, v, hash); err != nil {
		return
	}
	...
	job := validatorJob(v, provider, credentials, image, name, checks, workloadCheck)
	...
	if validationInProgress(v) {
		result.RequeueAfter = validationLeaseRenewInterval
	}
	return
}
...
func (r Reconciler) finalize(ctx context.Context, v *api.VirtualizationValidation) error {
	...
	if providerNamespace != v.Namespace {
		if err := r.releaseProviderLease(ctx, v); err != nil {
			return err
		}
...
func validationInProgress(v *api.VirtualizationValidation) bool {
	return v.Status.Phase == api.VirtualizationValidationPending || v.Status.Phase == api.VirtualizationValidationRunning
}

func providerLeaseName(v *api.VirtualizationValidation) string {
	identity := v.Namespace + "\x00" + v.Spec.ProviderRef.Name
	sum := sha256.Sum256([]byte(identity))
	return validationLeasePrefix + hex.EncodeToString(sum[:])[:12]
}

func (r Reconciler) acquireProviderLease(ctx context.Context, v *api.VirtualizationValidation) (bool, error) {
	key := types.NamespacedName{Namespace: v.Namespace, Name: providerLeaseName(v)}
	holder := string(v.UID)
	...
	if err := r.Get(ctx, key, lease); err != nil {
		...
		lease = &coordination.Lease{ObjectMeta: meta.ObjectMeta{Name: key.Name, Namespace: key.Namespace}, Spec: coordination.LeaseSpec{
			HolderIdentity:       ptr.To(holder),
			LeaseDurationSeconds: ptr.To(int32(validationLeaseDuration / time.Second)),
			AcquireTime:          &now,
			RenewTime:            &now,
		}}
	...
	lease.Spec.LeaseDurationSeconds = ptr.To(int32(validationLeaseDuration / time.Second))
...
func (r Reconciler) releaseProviderLease(ctx context.Context, v *api.VirtualizationValidation) error {
	lease := &coordination.Lease{}
	key := types.NamespacedName{Namespace: v.Namespace, Name: providerLeaseName(v)}
	if err := r.Get(ctx, key, lease); err != nil {
		if k8serr.IsNotFound(err) {
	...
	if lease.Spec.HolderIdentity == nil || *lease.Spec.HolderIdentity != string(v.UID) {
		return nil
	}
	return client.IgnoreNotFound(r.Delete(ctx, lease))
}

func leaseExpired(lease *coordination.Lease, now meta.MicroTime) bool {
	...
	return !expiresAt.After(now.Time)
}

// deleteSupersededResources removes objects created for a previous input hash.
// The current hash is kept so a normal reconcile observes, rather than replaces,
// its existing Job and progressive report.
func (r Reconciler) deleteSupersededResources(ctx context.Context, v *api.VirtualizationValidation, hash string) error {
	selector := client.MatchingLabels{ValidationLabel: string(v.UID)}
	options := []client.ListOption{client.InNamespace(v.Namespace), selector}
	currentHash := hash[:12]
	deleteObjects := func(objects []client.Object) error {
		for _, object := range objects {
			if !meta.IsControlledBy(object, v) || object.GetLabels()[ValidationInputLabel] == currentHash {
				continue
			}
			...
			log.Info("deleted superseded virtualization validation resource",
				"validation", v.Name, "inputHash", hash, "resource", object.GetName(),
	...
	secretObjects := make([]client.Object, len(secrets.Items))
	for i := range secrets.Items {
		secretObjects[i] = &secrets.Items[i]
	}
	if err := deleteObjects(secretObjects); err != nil {
		return err
	}
	serviceAccounts := &core.ServiceAccountList{}
	if err := r.List(ctx, serviceAccounts, options...); err != nil {
		return err
	}
	serviceAccountObjects := make([]client.Object, len(serviceAccounts.Items))
	for i := range serviceAccounts.Items {
		serviceAccountObjects[i] = &serviceAccounts.Items[i]
	}
	if err := deleteObjects(serviceAccountObjects); err != nil {
		return err
	}
	roles := &rbac.RoleList{}
	if err := r.List(ctx, roles, options...); err != nil {
		return err
	}
	roleObjects := make([]client.Object, len(roles.Items))
	for i := range roles.Items {
		roleObjects[i] = &roles.Items[i]
	}
	if err := deleteObjects(roleObjects); err != nil {
		return err
	}
	roleBindings := &rbac.RoleBindingList{}
	if err := r.List(ctx, roleBindings, options...); err != nil {
		return err
	}
	roleBindingObjects := make([]client.Object, len(roleBindings.Items))
	for i := range roleBindings.Items {
		roleBindingObjects[i] = &roleBindings.Items[i]
	}
	if err := deleteObjects(roleBindingObjects); err != nil {
		return err
	}
	configMaps := &core.ConfigMapList{}
	if err := r.List(ctx, configMaps, options...); err != nil {
		return err
	}
	configMapObjects := make([]client.Object, len(configMaps.Items))
	for i := range configMaps.Items {
		configMapObjects[i] = &configMaps.Items[i]
	}
	if err := deleteObjects(configMapObjects); err != nil {
		return err
	}
	jobs := &batch.JobList{}
	if err := r.List(ctx, jobs, options...); err != nil {
		return err
	}
	jobObjects := make([]client.Object, len(jobs.Items))
	for i := range jobs.Items {
		jobObjects[i] = &jobs.Items[i]
	}
	return deleteObjects(jobObjects)
}
...
func (r Reconciler) failed(v *api.VirtualizationValidation, reason, message string) {
	log.Info("virtualization validation failed", "validation", v.Name,
		"inputHash", v.Status.ObservedInputHash, "reason", reason, "message", message)
...
// VCV paths. Passing VCV's raw substring filters through the API would let
// callers select an empty or future mutating check set.
func providerProfileChecks(requested []string, validationNamespace string) ([]string, bool, error) {
	if len(requested) == 0 {
		requested = []string{"platform"}
	}
	platformCheck := false
	workloadCheck := false
	demoFailureCheck := false
	for _, check := range requested {
		...
		case "workload":
			workloadCheck = true
		case "demo-failure":
			demoFailureCheck = true
		default:
			return nil, false, fmt.Errorf("check %q is not supported by the Provider profile", check)
		}
	}
	if workloadCheck && validationNamespace == "" {
		return nil, false, fmt.Errorf("spec.validationNamespace is required when selecting the workload check")
	}
	checks := []string{}
	if platformCheck {
	...
	if workloadCheck {
		checks = append(checks, "50-openshift-virtualization.d/50-basic.d")
	}
	if demoFailureCheck {
		// PoC-only demo instrumentation. It must not be enabled in a product-facing validation flow.
		checks = append(checks, "99-mtv-demo.d/00-simulated-failure.d")
	}
	return checks, workloadCheck, nil
}
...
func inputHash(v *api.VirtualizationValidation, provider *api.Provider, secret *core.Secret, image string, checks []string) string {
	...
	sort.Strings(checks)
	input := strings.Join([]string{string(provider.UID), string(provider.Type()), provider.Spec.URL, secret.ResourceVersion, string(v.Spec.Profile), strings.Join(checks, ","), v.Spec.ValidationNamespace, image, v.Spec.RunNonce}, "\x00")
...
func validatorJob(v *api.VirtualizationValidation, provider *api.Provider, secret *core.Secret, image, name string, checks []string, workloadCheck bool) *batch.Job {
		...
		Spec: batch.JobSpec{
			...
			Template: core.PodTemplateSpec{Spec: core.PodSpec{
				...
				Containers: []core.Container{{
					Name:         "validator",
					Image:        image,
					Env:          validatorEnvironment(v, provider, name, checks, workloadCheck),
...
func validatorEnvironment(v *api.VirtualizationValidation, provider *api.Provider, name string, checks []string, workloadCheck bool) []core.EnvVar {
	env := []core.EnvVar{
	...
	if workloadCheck {
		env = append(env,
			core.EnvVar{Name: "VIRT_VALIDATE_NAMESPACE", Value: v.Spec.ValidationNamespace},
			core.EnvVar{Name: "VM_READY_TIMEOUT", Value: "5m"},
		)
	}
	return env
}
