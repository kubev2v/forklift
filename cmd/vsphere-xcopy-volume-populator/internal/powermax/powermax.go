...
func (p *PowermaxClonner) UnMap(initiatorGroup string, targetLUN populator.LUN, mappingContext populator.MappingContext) error {
	ctx := context.TODO()
	storageGroupId := getStorageGroupId(initiatorGroup)
	klog.Infof("removing volume ID %s from storage group %s", targetLUN.ProviderID, storageGroupId)

	_, err := p.client.RemoveVolumesFromStorageGroup(ctx, p.symmetrixID, storageGroupId, false, targetLUN.ProviderID)
	if err != nil {
