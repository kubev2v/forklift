...
func (p *RemoteEsxcliPopulator) Populate(vmId string, sourceVMDKFile string, pv PersistentVolume, hostLocker Hostlocker, progress chan<- uint64, xcopyUsed chan<- int, quit chan error) (errFinal error) {
	...
	if p.UseSSHMethod {
		...
		klog.V(2).Infof("Secure script ready: %s (UUID: %s)", scriptPath, scriptUUID)

		// Extract script filename for cleanup
		scriptName := fmt.Sprintf("%s-%s.py", secureScriptName, scriptUUID)
		...
		sshClient := vmware.NewSSHClient(scriptUUID)
		...
		klog.V(2).Infof("SSH connection established with restricted commands (script UUID: %s)", scriptUUID)
