...
var progressPatternSecure = regexp.MustCompile(`Clone:\s(\d+)%\sdone\.`)
...
// SecureSSHClient interface extends SSHClient with secure operations
type SecureSSHClient interface {
	Connect(hostname, username string, privateKey []byte) error
	StartSecureVmkfstoolsClone(sourceVMDK, targetLUN string) (*VmkfstoolsTask, error)
	GetSecureTaskStatus(taskId string) (*VmkfstoolsTask, error)
	CleanupSecureTask(taskId string) error
	Close() error
}

type ESXiSecureSSHClient struct {
	hostname   string
	username   string
...
func NewSecureSSHClient() SecureSSHClient {
	return &ESXiSecureSSHClient{}
}

func (c *ESXiSecureSSHClient) Connect(hostname, username string, privateKey []byte) error {
	c.hostname = hostname
	c.username = username
...
// executeSecureCommand executes a command using the secure SSH_ORIGINAL_COMMAND pattern
func (c *ESXiSecureSSHClient) executeSecureCommand(operation, arg1, arg2 string) (string, error) {
	if c.sshClient == nil {
	...
	klog.V(2).Infof("Executing secure SSH command: %s", sshCommand)
	...
	case <-ctx.Done():
		...
		session.Close()
		return "", fmt.Errorf("secure SSH command timed out after 60 seconds: %s", sshCommand)
	}

	if cmdErr != nil {
		klog.Warningf("Secure SSH command failed: %s, output: %s, error: %v", sshCommand, string(output), cmdErr)
		return string(output), cmdErr
	}

	klog.V(2).Infof("Secure SSH command succeeded: %s, output: %s", sshCommand, string(output))
	return string(output), nil
}

func (c *ESXiSecureSSHClient) StartSecureVmkfstoolsClone(sourceVMDK, targetLUN string) (*VmkfstoolsTask, error) {
	klog.Infof("Starting secure vmkfstools clone: source=%s, target=%s", sourceVMDK, targetLUN)

	output, err := c.executeSecureCommand("clone", sourceVMDK, targetLUN)
	if err != nil {
		return nil, fmt.Errorf("failed to start secure clone: %w", err)
	}

	klog.Infof("Received output from secure script: %s", output)

	// Parse the XML response from the secure script
	task, err := parseTaskResponse(output)
	if err != nil {
		return nil, fmt.Errorf("failed to parse clone response: %w", err)
	}

	klog.Infof("Started secure vmkfstools clone task %s with PID %d", task.TaskId, task.Pid)
	return task, nil
}

func (c *ESXiSecureSSHClient) GetSecureTaskStatus(taskId string) (*VmkfstoolsTask, error) {
	klog.V(2).Infof("Getting secure task status for %s", taskId)

	output, err := c.executeSecureCommand("status", taskId, "")
	if err != nil {
		return nil, fmt.Errorf("failed to get secure task status: %w", err)
	}
	...
	klog.V(2).Infof("Secure task %s status: PID=%d, ExitCode=%s, LastLine=%s",
		taskId, task.Pid, task.ExitCode, task.LastLine)

	return task, nil
}

func (c *ESXiSecureSSHClient) CleanupSecureTask(taskId string) error {
	klog.Infof("Cleaning up secure task %s", taskId)

	output, err := c.executeSecureCommand("cleanup", taskId, "")
	if err != nil {
		return fmt.Errorf("failed to cleanup secure task: %w", err)
	}
	...
	klog.Infof("Cleaned up secure task %s", taskId)
	return nil
}

func (c *ESXiSecureSSHClient) Close() error {
	if c.sshClient != nil {
		err := c.sshClient.Close()
		c.sshClient = nil
		klog.Infof("Closed secure SSH connection to %s", c.hostname)
		return err
	}
	return nil
}

// parseTaskResponse parses the XML response from the secure script
...
// EnableSecureSSHAccess enables SSH service on ESXi host and handles SSH key installation based on ESXi version
func EnableSecureSSHAccess(vmwareClient Client, host *object.HostSystem, privateKey, publicKey []byte, scriptPath string) error {
	ctx := context.Background()
	publicKeyStr := strings.TrimSpace(string(publicKey))

	klog.Infof("Enabling secure SSH access on ESXi host %s", host.Name())
	...
	if testSecureSSHConnectivity(hostIP, privateKey, "test") {
	...
	klog.Infof("Secure SSH access configured successfully on ESXi host %s", host.Name())
	return nil
}

// testSecureSSHConnectivity tests if we can connect via SSH and execute a restricted command
func testSecureSSHConnectivity(hostIP string, privateKey []byte, testCommand string) bool {
	klog.Infof("Testing secure SSH connectivity to %s", hostIP)

	client := NewSecureSSHClient()
	err := client.Connect(hostIP, "root", privateKey)
	if err != nil {
		klog.Infof("Secure SSH connectivity test failed to connect: %v", err)
		return false
	}
	defer client.Close()
	...
	output, err := client.(*ESXiSecureSSHClient).executeSecureCommand("status", "test-task-id", "")

	klog.Infof("Secure SSH test command output: '%s'", output)
	klog.Infof("Secure SSH test command error: %v", err)
	...
	klog.Infof("Secure SSH test completed successfully")
	return true
}
...
func ParseProgress(lastLine string) (uint, bool) {
	allMatches := progressPatternSecure.FindAllStringSubmatch(lastLine, -1)
	if len(allMatches) > 0 {
