...
var _ = Describe("DestinationClient", func() {
	...
	Describe("SetPopulatorCrOwnership", func() {
		It("should set ownership reference on populator CR", func() {
			// Setup
			vmdkKey := "disk-1"
			...
			// Execute
			err := destClient.SetPopulatorCrOwnership()

			// Assert
			Expect(err).NotTo(HaveOccurred())

			// Verify ownership reference was set
			updatedPopulator := &v1beta1.VSphereXcopyVolumePopulator{}
			err = destClient.Destination.Client.Get(context.TODO(), client.ObjectKey{
				Name:      "populator-1",
				Namespace: "test",
			}, updatedPopulator)
			Expect(err).NotTo(HaveOccurred())
			Expect(updatedPopulator.OwnerReferences).NotTo(BeEmpty())
			Expect(updatedPopulator.OwnerReferences[0].Name).To(Equal("test-pvc"))
		})

		It("should continue without error when PVC is not found", func() {
			// Setup - populator CR without matching PVC
			populator := &v1beta1.VSphereXcopyVolumePopulator{
				ObjectMeta: meta.ObjectMeta{
					Name:      "populator-orphan",
					Namespace: "test",
					Labels: map[string]string{
						"migration": "123",
						"vmdkKey":   "orphan-disk",
