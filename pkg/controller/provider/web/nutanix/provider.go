...
func (h ProviderHandler) Get(ctx *gin.Context) {
	...
	if h.Provider.Type() != api.Nutanix {
...
func (h *ProviderHandler) ListContent(ctx *gin.Context) (content []interface{}, err error) {
	...
	for _, collector := range list {
		if p, cast := collector.Owner().(*api.Provider); cast {
			if p.Type() != api.Nutanix || (ns != "" && ns != p.Namespace) {
				continue
			}
...
func (h *ProviderHandler) AddDerived(r *Provider) (err error) {
	...
	db := h.Collector.DB()
	// Cluster
	n, err = db.Count(&model.Cluster{}, nil)
	if err != nil {
		return
	}
	r.ClusterCount = n
	// Host
	n, err = db.Count(&model.Host{}, nil)
	if err != nil {
		return
	}
	r.HostCount = n
	...
	r.NetworkCount = n
	// StorageContainer
	n, err = db.Count(&model.StorageContainer{}, nil)
	if err != nil {
		return
	}
	r.StorageContainerCount = n
	// Image
	n, err = db.Count(&model.Image{}, nil)
	if err != nil {
		return
	}
	r.ImageCount = n
