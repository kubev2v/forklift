...
func (r *Collector) fetchPowerState(ctx context.Context, vm *armcompute.VirtualMachine) string {
	...
	resourceGroup, err := auth.ParseResourceGroup(*vm.ID)
	if err != nil {
		r.log.V(2).Info("Failed to parse VM resource group, powerState will be unknown", "vm", *vm.Name, "error", err)
		return "unknown"
	}
	iv, err := r.client.GetVMInstanceView(ctx, resourceGroup, *vm.Name)
	if err != nil {
