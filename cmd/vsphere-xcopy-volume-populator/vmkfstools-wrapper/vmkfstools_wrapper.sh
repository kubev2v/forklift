...
LOG_FILE="/var/log/vmkfstools-wrapper.log"
...
generate_task_id() {
    # Use /dev/urandom (available on ESXi)
    if [ -r /dev/urandom ]; then
        # Read 16 bytes and convert to hex using od
        # Note: tr not available on ESXi, use sed instead
        local hex
        ...
        # Last resort fallback: timestamp-pid-hash
        # Use md5sum if available to create more randomness
        ...
        if command -v md5sum >/dev/null 2>&1; then
            ...
            # Absolutely last resort: just use timestamp-pid
            echo "${seed}"
        fi
    fi
}
...
clone() {
    ...
    (
        # setsid creates new session (Python's os.setsid equivalent)
        ...
        echo "$(basename "${target}")" > "${task_dir}/targetLun.tmp" && mv "${task_dir}/targetLun.tmp" "${task_dir}/targetLun"

        local json_result="{\"taskId\": \"${task_id}\", \"pid\": ${task_pid}}"
...
was_xcopy_used() {
    local target_lun="$1"
    # Remove whitespace using sed (BusyBox compatible)
...
task_get() {
    ...
    if [ -z "${task_id}" ]; then
        local json_result="{\"error\": \"-i (task-id) is required for task-get\"}"
    ...
    if [ ! -d "${task_dir}" ]; then
        local json_result="{\"taskId\": \"${task_id}\", \"error\": \"Task directory ${task_dir} not found\"}"
        xml_output "1" "${json_result}"
        return 1
    fi
    ...
    pid=$(cat "${task_dir}/pid" 2>/dev/null) || {
        xml_output "1" "{\"taskId\": \"${task_id}\", \"error\": \"Failed to read PID\"}"
        return 1
    }
    ...
    local xcopy_used="null"
    local target_lun
    if [ -f "${task_dir}/targetLun" ]; then
        target_lun=$(cat "${task_dir}/targetLun" 2>/dev/null)
        if [ -n "${target_lun}" ]; then
            # Try to determine xcopy usage, but keep as null if vsish fails
            local xcopy_result
            # Don't redirect stderr - was_xcopy_used logs to stderr, we only want stdout
            if xcopy_result=$(was_xcopy_used "${target_lun}"); then
                local xcopy_bool
                local xclone_writes
                xcopy_bool=$(echo "${xcopy_result}" | cut -d' ' -f1)
                xclone_writes=$(echo "${xcopy_result}" | cut -d' ' -f2)

                log_info "XCOPY used: ${xcopy_bool}, clone write ops: ${xclone_writes}"
                xcopy_used="${xcopy_bool}"
            else
                log_warning "Failed to determine xcopy usage, defaulting to null"
            fi
        fi
    else
        log_warning "Failed to determine xcopy usage: targetLun file not found, defaulting to null"
    fi
    ...
    local json_result
    if [ -n "${exitcode}" ]; then
        json_result="{\"taskId\": \"${task_id}\", \"pid\": ${pid}, \"exitCode\": \"${exitcode}\", \"lastLine\": \"${escaped_last_line}\", \"xcopyUsed\": ${xcopy_used}, \"stdErr\": \"${escaped_stderr}\"}"
    else
        json_result="{\"taskId\": \"${task_id}\", \"pid\": ${pid}, \"exitCode\": \"\", \"lastLine\": \"${escaped_last_line}\", \"xcopyUsed\": ${xcopy_used}, \"stdErr\": \"${escaped_stderr}\"}"
    fi
...
task_clean() {
    ...
    if [ -z "${task_id}" ]; then
        local json_result="{\"error\": \"-i (task-id) is required for task-clean\"}"
        xml_output "1" "${json_result}"
        return 1
    fi
    ...
    # Retry mechanism for directory deletion
    local rm_error_file="/tmp/rmdir-error-$$.log"
    local retry_count=0
    local max_retries=3

    while [ ${retry_count} -lt ${max_retries} ]; do
        if rm -rf "${task_dir}" 2>"${rm_error_file}"; then
        ...
        log_warning "Failed to remove ${task_dir} (attempt ${retry_count}/${max_retries})"
        sleep 1
    done

    # Capture diagnostic info and try individual file deletion
    local rm_error
    rm_error=$(cat "${rm_error_file}" 2>/dev/null || echo "unknown error")
    log_error "failed to remove task directory ${task_dir}: ${rm_error}"
    ...
    rm -f "${rm_error_file}"
    xml_output "1" "failed to remove task directory ${task_dir}: ${rm_error}"
    return 1
}
