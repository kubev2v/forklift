...
func TestNewOntapImporter(t *testing.T) {
	...
	for _, tt := range validTests {
		imp, err := NewOntapImporter(tt.host, "user", "pass", "svm", "uuid", "", true)
		if err != nil {
	...
	for _, host := range invalidHosts {
		_, err := NewOntapImporter(host, "user", "pass", "svm", "uuid", "", true)
		if err == nil {
	...
	// Missing SVM
	_, err := NewOntapImporter("https://host", "user", "pass", "", "uuid", "", true)
	if err == nil {
		t.Fatal("expected error for empty SVM, got nil")
	}

	// Missing backend UUID
	_, err = NewOntapImporter("https://host", "user", "pass", "svm", "", "", true)
	if err == nil {
...
func TestResolveVVol(t *testing.T) {
	...
	_, err := imp.Resolve(backing)
	if err == nil {
		t.Fatal("expected error for VVol, got nil")
	}
	if !strings.Contains(err.Error(), "does not yet support VVol") {
