...
func NewOntapImporter(host, user, pass, svm, backendUUID, driverType string, skipSSL bool) (*OntapImporter, error) {
...
func (o *OntapImporter) Resolve(backing *resolver.DiskBacking) (map[string]string, error) {
	...
	case resolver.DiskTypeVVol:
		return nil, fmt.Errorf("ONTAP CSI import does not yet support VVol disks")
...
func (o *OntapImporter) resolveRDM(deviceName string) (map[string]string, error) {
	...
	importName, err := formatImportName(lunPath, o.driverType)
	if err != nil {
		return nil, err
	}
