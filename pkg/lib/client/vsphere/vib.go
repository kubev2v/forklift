...
//go:generate go run go.uber.org/mock/mockgen -destination=mocks/vib_ensurer_mock.go -package=vsphere_mocks . VIBEnsurer
...
// EnsureVib checks the VIB version and installs if needed
...
// GetHostDatastore retrieves the first available datastore name from a given ESXi host
