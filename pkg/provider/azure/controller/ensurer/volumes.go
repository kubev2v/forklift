...
func (r *Ensurer) EnsurePVCs(vm *planapi.VMStatus) error {
	...
	for i, disk := range disks {
		...
		pvc.Annotations[azure.AnnVolumeSnapshot] = fmt.Sprintf("%s-snap-%d", vm.Name, i)
		...
		err = r.Destination.List(ctx, existing,
		...
		err = r.Destination.Create(ctx, pvc)
		if err != nil {
			return liberr.Wrap(err)
		}
...
func (r *Ensurer) CheckPVCsBound(vm *planapi.VMStatus) (bool, error) {
	...
	err := r.Destination.List(ctx, pvcList,
...
func (r *Ensurer) InjectOwnerReferences(vm *planapi.VMStatus) error {
	...
	for _, pvc := range pvcList.Items {
		...
		err := r.Destination.Get(ctx, types.NamespacedName{
			Name:      vsName,
			Namespace: pvc.Namespace,
		}, vs)
		if err != nil {
		...
		err = r.Destination.Update(ctx, vs)
		if err != nil {
			return liberr.Wrap(err)
		}
...
func (r *Ensurer) DeleteVolumeSnapshots(vm *planapi.VMStatus) error {
	...
	err := r.Destination.List(ctx, vsList,
		client.InNamespace(r.Plan.Spec.TargetNamespace),
		client.MatchingLabels{azure.LabelVMID: vm.ID},
	)
	if err != nil {
	...
	for i := range vsList.Items {
		vs := &vsList.Items[i]
		err = r.Destination.Delete(ctx, vs, &client.DeleteOptions{
...
func (r *Ensurer) DeleteVolumeSnapshotContents(vm *planapi.VMStatus) error {
	...
	err := r.Destination.List(ctx, vscList,
		client.MatchingLabels{azure.LabelVMID: vm.ID},
	)
	if err != nil {
		return liberr.Wrap(err)
	...
	for i := range vscList.Items {
		vsc := &vscList.Items[i]
		err = r.Destination.Delete(ctx, vsc)
		if err != nil && !k8serr.IsNotFound(err) {
			return liberr.Wrap(err)
		}
