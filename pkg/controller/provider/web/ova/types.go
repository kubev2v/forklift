...
// These allow existing code to continue using ova.VM, ova.Network, etc.
...
// Resolver for OVA providers - pre-configured with OVA config.
...
// Finder for OVA providers - pre-configured with OVA config.
type Finder struct {
	ovfbase.Finder
}

// NewFinder creates a new OVA finder with the correct config.
