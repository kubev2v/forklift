...
// OpenStack Address/NIC Fields
const (
	// OpenStack extension field for IP type (fixed or floating)
	OSExtIPsType = "OS-EXT-IPS:type"
	// OpenStack extension field for MAC address
	OSExtIPsMacAddr = "OS-EXT-IPS-MAC:mac_addr"
)

// Auto-generated NAD labels and annotations
const (
	LabelAutoGenerated    = "forklift.konveyor.io/auto-generated"
	LabelSourceNAD        = "forklift.konveyor.io/source-nad"
	AnnotationDescription = "forklift.konveyor.io/description"
	AnnotationCounter     = "forklift.konveyor.io/counter"
)
...
// Ensures a numbered auto-generated NAD for handling multiple NICs on the same network.
// This creates numbered NADs (e.g., "my-nad-auto-1", "my-nad-auto-2") that can be reused across migrations.
// The counter parameter determines which numbered NAD to create/retrieve.
func (r *Builder) ensureNumberedAutoNAD(originalNADName, namespace string, counter int) (string, error) {
	if namespace == "" {
		return "", liberr.New("namespace cannot be empty for auto-NAD generation")
	}
	if originalNADName == "" {
		return "", liberr.New("original NAD name cannot be empty")
	}

	autoNADName := fmt.Sprintf("%s-auto-%d", originalNADName, counter)

	// Check if the auto-generated NAD already exists
	autoNAD := &net.NetworkAttachmentDefinition{}
	err := r.Client.Get(context.TODO(),
		client.ObjectKey{Namespace: namespace, Name: autoNADName},
		autoNAD)

	if err == nil {
		// NAD already exists, reuse it
		r.Log.V(1).Info("Reusing existing numbered auto-generated NAD",
			"nad", autoNADName,
			"namespace", namespace,
			"counter", counter)
		return autoNADName, nil
	}

	if !k8serr.IsNotFound(err) {
		return "", liberr.Wrap(err, "failed to check for numbered auto-generated NAD",
			"nad", autoNADName,
			"counter", counter)
	}

	// NAD doesn't exist, need to create it by cloning the original
	r.Log.Info("Creating numbered auto-generated NAD by cloning original",
		"originalNAD", originalNADName,
		"autoNAD", autoNADName,
		"namespace", namespace,
		"counter", counter)

	// Get the original NAD
	originalNAD := &net.NetworkAttachmentDefinition{}
	err = r.Client.Get(context.TODO(),
		client.ObjectKey{Namespace: namespace, Name: originalNADName},
		originalNAD)
	if err != nil {
		return "", liberr.Wrap(err, "failed to get original NAD for cloning",
			"nad", originalNADName)
	}

	// Clone the NAD with a numbered name
	newNAD := &net.NetworkAttachmentDefinition{
		ObjectMeta: meta.ObjectMeta{
			Name:      autoNADName,
			Namespace: namespace,
			Labels: map[string]string{
				LabelAutoGenerated: "true",
				LabelSourceNAD:     originalNADName,
			},
			Annotations: map[string]string{
				AnnotationDescription: fmt.Sprintf("Auto-generated NAD #%d cloned from %s to support multiple NICs", counter, originalNADName),
				AnnotationCounter:     strconv.Itoa(counter),
			},
		},
		Spec: originalNAD.Spec, // Clone the original NAD spec
	}

	// Create the new NAD
	err = r.Client.Create(context.TODO(), newNAD)
	if err != nil {
		// Handle race condition: another migration might have created the same NAD concurrently
		if k8serr.IsAlreadyExists(err) {
			r.Log.V(1).Info("NAD was created concurrently by another process, reusing it",
				"nad", autoNADName,
				"namespace", namespace,
				"counter", counter)
			return autoNADName, nil
		}
		return "", liberr.Wrap(err, "failed to create numbered auto-generated NAD",
			"nad", autoNADName,
			"counter", counter)
	}

	r.Log.Info("Successfully created numbered auto-generated NAD",
		"nad", autoNADName,
		"namespace", namespace,
		"counter", counter)
...
// NICInfo represents a unique NIC identified by its MAC address
type NICInfo struct {
	Entry interface{}
	MAC   string
	IPs   []string
...
// groupNICsByMAC groups network interface entries by MAC address to identify unique NICs.
// Multiple entries with the same MAC address represent a single NIC with multiple IPs.
func (r *Builder) groupNICsByMAC(nics []interface{}) map[string]*NICInfo {
	nicsByMAC := make(map[string]*NICInfo)

	for _, nicEntry := range nics {
		if m, ok := nicEntry.(map[string]interface{}); ok {
			// Skip floating IPs
			if ipType, ok := m[OSExtIPsType]; ok {
			...
			// Track unique NICs by MAC address and collect all IPs
			if nicsByMAC[macAddr] == nil {
				nicsByMAC[macAddr] = &NICInfo{
					Entry: nicEntry,
					MAC:   macAddr,
					IPs:   []string{},
				}
			...
			if ipAddr != "" {
				nicsByMAC[macAddr].IPs = append(nicsByMAC[macAddr].IPs, ipAddr)
			}
		}
	}
...
// processNIC creates KubeVirt network and interface configurations for a single NIC
func (r *Builder) processNIC(
	nicInfo *NICInfo,
	networkPair *api.NetworkPair,
	vm *model.Workload,
	vmNetworkName string,
	hasUDN bool,
	numNetworks int,
	usedDestinations map[string]bool,
	autoNADCounters map[string]int,
) (kNetwork cnv.Network, kInterface cnv.Interface, err error) {
	nic := nicInfo.Entry

	// Check if this destination has already been used
	var destIdentifier string
	var destNADName string
	var destNADNamespace string

	if networkPair.Destination.Type == Pod {
		destIdentifier = "pod"
		destNADName = ""
		destNADNamespace = ""
		...
		if usedDestinations[destIdentifier] {
			// Save the original NAD name before modification
			originalDestNADName := destNADName

			// Get the counter for this original NAD
			originalKey := path.Join(destNADNamespace, originalDestNADName)
			autoNADCounters[originalKey]++
			counter := autoNADCounters[originalKey]

			r.Log.Info("Destination NAD already used, auto-generating numbered NAD",
				"originalNAD", originalDestNADName,
				"namespace", destNADNamespace,
				"counter", counter,
				"network", vmNetworkName,
				"mac", nicInfo.MAC)

			autoNADName, genErr := r.ensureNumberedAutoNAD(originalDestNADName, destNADNamespace, counter)
			if genErr != nil {
				err = liberr.Wrap(genErr, "failed to auto-generate numbered NAD",
					"originalNAD", originalDestNADName,
					"namespace", destNADNamespace,
					"counter", counter)
				return
			...
			// Update to use the auto-generated NAD
			destNADName = autoNADName
			destIdentifier = path.Join(destNADNamespace, destNADName)
			r.Log.V(1).Info("Using auto-generated numbered NAD",
				"autoNAD", autoNADName,
				"counter", counter)
		}
	}

	// Mark this destination as used
	...
	case Multus:
		kNetwork.Multus = &cnv.MultusNetwork{
			NetworkName: path.Join(destNADNamespace, destNADName),
		}
...
func (r *Builder) mapNetworks(vm *model.Workload, object *cnv.VirtualMachineSpec) (err error) {
	...
	numNetworks := 0
	// Track which destination NADs have been used to detect duplicates
	usedDestinations := make(map[string]bool) // key: namespace/name for Multus, "pod" for pod network
	// Track auto-NAD counters per original NAD to support multiple NICs on same network
	autoNADCounters := make(map[string]int) // key: namespace/originalNADName, value: counter
	...
	for vmNetworkName, vmAddresses := range vm.Addresses {
		if nics, ok := vmAddresses.([]interface{}); ok {
			...
			if !found {
				err = liberr.New("no network map for vm network",
					"vm", vm.Name,
					"network", vmNetworkName,
					"networkID", vmNetworkID)
				return
			}
			...
			// Group NICs by MAC address to identify unique NICs
			nicsByMAC := r.groupNICsByMAC(nics)

			// Process each unique NIC (by MAC address)
			for _, nicInfo := range nicsByMAC {
				kNetwork, kInterface, processErr := r.processNIC(
					nicInfo,
					networkPair,
					vm,
					vmNetworkName,
