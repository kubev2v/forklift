...
Deploy an MCP (Model Context Protocol) server as a dedicated pod, gated by its own `feature_mcp_server` feature flag, expose it on a dedicated HTTP service, and provide opt-in OpenShift Lightspeed integration so that AI assistants can interact with Forklift to help users plan and execute VM migrations. The MCP server requires OpenShift with Lightspeed installed (`ols.openshift.io` API group present).
...
- Vanilla Kubernetes support (the MCP server feature is OpenShift-only and requires Lightspeed)
...
The Service is `ClusterIP` with no external Route. The MCP server pod is deployed when `feature_mcp_server` is enabled on the `ForkliftController` CR. It requires OpenShift (not vanilla k8s) and the `ols.openshift.io` API group to be present on the cluster (i.e., Lightspeed must be installed).
...
A new operator setting `mcp_server_lightspeed_integration` (default `true`) controls automatic registration with OpenShift Lightspeed.
...
3. Logs a warning if `MCPServer` is not in `spec.featureGates`
4. If `mcp_server_lightspeed_set_mcp_gate` is `true`, adds `MCPServer` to `spec.featureGates`
5. Appends the new MCP server entry and applies the result

The `MCPServer` feature gate must be present in OLSConfig for Lightspeed to invoke MCP tools. By default the operator does **not** modify `featureGates` -- it only logs a warning with instructions for the admin. Set `mcp_server_lightspeed_set_mcp_gate: true` to let the operator add the gate automatically.
...
spec:
  featureGates:
    - MCPServer  # only added when mcp_server_lightspeed_set_mcp_gate is true
  mcpServers:
    - name: kubectl-mtv
...
When `mcp_server_lightspeed_integration` is set back to `false` (or `feature_mcp_server` is disabled), the operator performs a read-modify-write to remove only the `kubectl-mtv` entry from the `mcpServers` list, preserving any other MCP server registrations.
...
|---|---|---|
| `feature_mcp_server` | `false` | Enable MCP server deployment (requires OpenShift with Lightspeed) |
| `mcp_server_lightspeed_integration` | `true` | Register MCP server with OpenShift Lightspeed |
| `mcp_server_lightspeed_set_mcp_gate` | `false` | Add `MCPServer` to OLSConfig `featureGates` when registering with Lightspeed |
...
| `mcp_server_container_limits_cpu` | `1000m` | MCP server CPU limit |
| `mcp_server_container_limits_memory` | `512Mi` | MCP server memory limit |
| `mcp_server_container_requests_cpu` | `100m` | MCP server CPU request |
| `mcp_server_container_requests_memory` | `256Mi` | MCP server memory request |
...
- With `mcp_server_lightspeed_integration: true`, verify the OLSConfig is patched correctly (featureGates and mcpServers)
- With `mcp_server_lightspeed_integration: false`, verify the `kubectl-mtv` entry is removed from OLSConfig without affecting other entries
...
- Verify disabling `feature_mcp_server` cleans up the MCP server resources
- Verify disabling `feature_cli_download` cleans up the CLI download resources independently

### Upgrade / Downgrade Strategy

- **Upgrade**: When `feature_mcp_server` is enabled, a new MCP server pod is deployed. The MCP Services are created automatically. No manual action required.
