...
func noopProbe(_ utils.CommandBuilder, _ []string, _ []string, _ string) (*api.GuestInfo, error) {
	return &api.GuestInfo{OS: api.GuestOS{Family: api.OSFamilyLinux}}, nil
}

func noopCommitFiles(_ utils.CommandBuilder, _ []string, _ []string, _ string, _ []api.FileAction) error {
	return nil
}
...
func TestRun(t *testing.T) {
	...
	t.Run("missing Config returns error", func(t *testing.T) {
		t.Parallel()
		err := Run(Options{Disks: []string{"/dev/sda"}})
	...
	t.Run("empty Disks returns error", func(t *testing.T) {
		t.Parallel()
		err := Run(Options{Config: &config.AppConfig{}})
	...
	t.Run("probe failure propagated", func(t *testing.T) {
		...
		err := Run(Options{
			...
			FileSystem: noopFS,
			probeGuest: func(_ utils.CommandBuilder, _ []string, _ []string, _ string) (*api.GuestInfo, error) {
	...
	t.Run("file actions trigger commitFiles", func(t *testing.T) {
		...
		err := Run(Options{
			...
			commitFiles: func(_ utils.CommandBuilder, _ []string, _ []string, _ string, files []api.FileAction) error {
	...
	t.Run("commitFiles not called when no file actions", func(t *testing.T) {
		...
		err := Run(Options{
			...
			commitFiles: func(_ utils.CommandBuilder, _ []string, _ []string, _ string, _ []api.FileAction) error {
				filesCalled.Store(true)
				return nil
			},
			commitScripts: noopCommitScripts,
		...
		err := Run(Options{
			...
			plugins: []api.Plugin{
				&stubPlugin{
					name: "test/runner",
					actions: &api.Actions{
						Execs: []api.ExecAction{{Type: api.ActionFirstboot, Value: "/tmp/init.sh"}},
					},
				},
			},
			commitFiles: noopCommitFiles,
	...
	t.Run("commitFiles error propagated", func(t *testing.T) {
		...
		if err == nil || !strings.Contains(err.Error(), "guestfish apply:") {
			t.Fatalf("expected 'guestfish apply:' wrapper, got: %v", err)
		}
	})
		...
		err := Run(Options{
			...
			plugins: []api.Plugin{
				&stubPlugin{
					name: "test/runner",
					actions: &api.Actions{
						Execs: []api.ExecAction{{Type: api.ActionRun, Value: "/tmp/fix.sh"}},
					},
				},
			},
			commitFiles: noopCommitFiles,
